...
func competitionScoreHandler(c echo.Context) error {
	...
	mu := getLockByTennatID(v.tenantID)
	mu.Lock()
	defer mu.Unlock()
...
func competitionRankingHandler(c echo.Context) error {
	...
	if _, err := adminDB.ExecContext(
		...
		return fmt.Errorf(
			"error Insert visit_history_simple: playerID=%s, tenantID=%d, competitionID=%s, createdAt=%d, updatedAt=%d, %w",
